...
func _drag_position_x(
	...
	EDITOR_DRIVER.change_point_property(
		inspector,
...
func _test_position_x_reorder_undo_redo_follows_the_selected_resource() -> void:
	...
	_expect(editor.selected_index == 1 and int(inspector.get("_selected_point_index")) == 1, "Position-X Undo did not synchronize graph and Inspector selection")
	_expect(int(inspector.get("_selected_point_resource_id")) == selected.get_instance_id(), "Position-X Undo changed the selected Resource")
	_expect(StringName(inspector.get("_selected_point_property_name")) == &"position", "Position-X Undo lost the selected property")
	history.redo()
	_expect(curve.points[2] == selected, "Position-X Redo did not return the selected Resource to P3")
	_expect(editor.selected_index == 2 and int(inspector.get("_selected_point_index")) == 2, "Position-X Redo did not synchronize graph and Inspector selection")
